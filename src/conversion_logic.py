...
def classify_conversion_rows(df, mapping=None, conversion_quality=None):
    ...
    if lookup.empty:
        ...
        out = out.merge(lookup, on="conversion_action", how="left")
...
def build_conversion_lookup(mapping=None, conversion_quality=None):
    ...
    for source_name, source in [("map_conversion_actions", mapping), ("model_conversion_quality", conversion_quality)]:
        ...
        frame = source.copy()
        bucket_col = next(
            (col for col in ["standardized_conversion_bucket", "standardized_bucket", "conversion_bucket", "metric_bucket", "conversion_type"] if col in frame.columns),
            None,
        )
        if bucket_col is None:
            continue
        frame["standardized_conversion_bucket"] = frame[bucket_col].apply(normalize_mapped_bucket)
        ...
        frames.append(frame[["conversion_action", "standardized_conversion_bucket", "conversion_metric_source", "conversion_mapping_status"]])
    if not frames:
        return pd.DataFrame()
    return pd.concat(frames, ignore_index=True).drop_duplicates("conversion_action", keep="first")
